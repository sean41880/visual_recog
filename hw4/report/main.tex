...
(dim$=64$, 200 epochs, 8-way test-time augmentation) achieves
\textbf{26.01 dB} PSNR on the validation set. Further scaling the model
to dim$=80$ is investigated as an additional experiment.
\end{abstract}
...
run-4 & 64 & 128 & 200 & 25.81 dB & \textbf{26.01 dB} \\
...
\textbf{[INSERT TRAINING CURVE HERE]}\\
Plot: epoch vs.\ val PSNR for run-4 (dim=64, 200 epochs).\\
Data: \texttt{checkpoints/run4/train\_log.csv}
\vspace{2cm}}}
\caption{Validation PSNR over 200 training epochs for the best model
(dim$=64$, $128\!\times\!128$ patches). The curve shows steady improvement
with cosine LR decay reaching a plateau around epoch~150.}
...
% -----------------------------------------------------------------
\textit{[Run-7 (dim$=80$) is currently training. Update this section with
the final val PSNR and TTA PSNR once training completes.
Expected: ${\sim}26.3$--$26.6$ dB based on observed scaling trend.]}
...
26.01 dB PSNR with 8-way TTA. Model scaling experiments confirm that
capacity remains a limiting factor, and dim$=80$ is expected to push
performance further.
