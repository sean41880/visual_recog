...
\title{All-in-One Image Restoration with PromptIR: Joint Rain and Snow Removal via Prompt-Guided Transformers}

\author{Tzu-Hsiang Chou (314552036)}
...
\textbf{26.75 dB} PSNR on the validation set and \textbf{27.59 dB} on the
CodaBench test set. Model scaling from dim$=64$ to dim$=80$ is investigated
as an additional experiment, yielding a $+0.74$ dB improvement.
\end{abstract}
...
on Restormer~\cite{zamir2022restormer}.
...
\begin{table}[H]
\centering
\caption{PSNR across runs. TTA: 8-way test-time augmentation
(all combinations of H-flip, V-flip, rot90). Test PSNR from CodaBench.}
\label{tab:results}
\begin{tabular}{lllllrr}
\toprule
Run & dim & Patch & Epochs & Val PSNR & Val PSNR (TTA) & Test PSNR \\
\midrule
...
run-7 & 80 & 128 & 200 & 26.53 dB & \textbf{26.75 dB} & \textbf{27.59 dB} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{scoreboard.png}
\caption{CodaBench leaderboard screenshot showing the test PSNR of our
best submission (dim$=80$, 8-way TTA): \textbf{27.59 dB}.}
...
Fig.~\ref{fig:qualitative} shows two validation examples. The top row shows
a challenging snow scene (22.83 dB) where the model successfully removes
heavy snow while preserving structural detail. The bottom row shows a rain
example (30.94 dB) where fine rain streaks are cleanly removed with minimal
artefacts.

\begin{figure}[H]
\centering
\includegraphics[width=0.97\textwidth]{qualitative.png}
\caption{Qualitative restoration results. Each row shows: degraded input
(left), our model output (centre), and ground truth (right).
Top: snow removal (22.83 dB). Bottom: rain removal (30.94 dB).}
...
\emph{slightly below} the baseline run-4 (25.81 / 26.01 dB), as shown in
Fig.~\ref{fig:ablation} (right). We conclude that the benefit of larger
context is outweighed by the smaller effective batch size and reduced number
of update steps within 60 fine-tune epochs.
...
respectively. Fig.~\ref{fig:ablation} (left) shows all three scales
converging to distinctly different PSNR levels, confirming that capacity
is the primary bottleneck.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{ablation_curves.png}
\caption{Left: Val PSNR for model scale experiment (dim=48/64/80). Each
step up in capacity yields consistent improvement.
Right: Val PSNR for patch size experiment --- run-6 (192px fine-tune)
does not surpass run-4 (128px from scratch) within 60 fine-tune epochs.}
\label{fig:ablation}
\end{figure}
...
frequency-targeted artifact suppression. Our best submission achieves \textbf{27.59 dB} PSNR on the CodaBench test
set (26.75 dB on validation) with 8-way TTA (dim$=80$, run-7), a $+0.74$ dB
